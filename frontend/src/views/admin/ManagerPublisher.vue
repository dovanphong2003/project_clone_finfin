...
// cpt modal right
import ModalEditPublisher from '@/modules/ManagerPublisher/Modals/ModalEditPublisher.vue'
...
import CreatePublisher from '@/modules/ManagerPublisher/CreatePublisher.vue'
import axiosInstance from '@/services/axiosService'
const headers: Header[] = [
  { text: 'Hành động', value: 'handle', width: 130 },
  { text: 'ID', value: 'publisher_id', width: 130 },
  { text: 'Tên', value: 'name', sortable: true, width: 130 },
  { text: 'Địa chỉ', value: 'address', width: 150 },
  { text: 'Số điện thoại', value: 'contact_number', width: 150 },
  { text: 'Email', value: 'email', width: 150 },
  { text: 'Website', value: 'website', width: 150 },
  { text: 'Ngày tạo', value: 'createdAt', width: 200 },
  { text: 'Ngày cập nhật', value: 'updatedAt', width: 200 }
]
const store = useListPublisherStore()
// call api get data all Publisher
const fncGetAllPublisher = async () => {
  try {
    const result = await axiosInstance.get('/api/Publisher')
...
// onclick
const PublisherData = ref<IPublisher>({
  publisher_id: 0,
  name: '',
  address: '',
  email: '',
...
const showRow = async (val: ClickRowArgument) => {
  if (checkDelete.value) {
    const result = await axiosInstance.delete('/api/Publisher', {
      params: { id: val.publisher_id }
    })
    if (result.data.isSuccess) {
      handleLoadingNotication('Xóa thành công', 500, 'top-center')
      store.deletePublisher(val.publisher_id)
    } else {
    ...
    PublisherData.value = val as any
  }
}
</script>
<template>
  <div class="main-manager_User">
    <div class="table">
      ...
      >
        <ModalEditPublisher
          v-if="activeModalVertical && PublisherData.publisher_id"
          :is="ModalEditPublisher"
          :objPublisher="PublisherData"
        ></ModalEditPublisher>
      </CptModalRight>
    </div>
  </div>
